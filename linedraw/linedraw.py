...
no_cv = False
default_export_path = os.path.abspath(os.getcwd()) + "/linedraw/output/out.svg"
draw_contours = True
draw_hatch = True
show_bitmap = False
resolution = 1024
...
def sketch(path, output: str):
    ...
    if IM is None:
        raise Exception("no image to open")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--contour_simplify",
        dest="contour_simplify",
        default=contour_simplify,
        action="store",
        nargs="?",
        type=int,
    ...
    no_cv = args.no_cv
    sketch(args.input_path, export_path)
